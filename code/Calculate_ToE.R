...
for(i in 1:length(extract_fls)){
  ...
  dvel<-ddply(dat,.(species,NAME,cell_area,mod,climate_proj,year),.fun=mmfun,.progress='text')
  l[[i]]<-dvel
...
ToEs<-ddply(df2,.(species,NAME,cell_area,mod,climate_proj),.fun=findToE,.progress='text')
head(ToEs)
