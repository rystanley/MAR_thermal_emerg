#This code will extract the time series for temperature data from the CMIP rasters and average among models. 
...
}else{message("Loading previous raster masks, if anything has changed to the polyons or climate projections the loop above should be re-run and the rasters saved.")
      load("data/raster_masks.RData") # if it is done we only need to do it once, so just load the previous results. 
...
#read in data and create brick (month (12) x years (86))
  ...
  save(cmip_extracts,file=paste0("output/climate_extracts/climate_extracts_",tolower(mod),"_",gsub("\\.","-",climate_proj),".RData"))
  save(cmip_extracts_shapes,file=paste0("output/climate_extracts/climate_extracts_shape_",tolower(mod),"_",gsub("\\.","-",climate_proj),".RData"))
